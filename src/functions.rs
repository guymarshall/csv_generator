...
pub fn generate_student_csv(filename: &str, field_headings: Vec<&str>, data: Vec<Vec<(String, &str, &str, &str, &str)>>) {
    let path: &Path = Path::new(filename);
    let mut file: File = match File::create(&path) {
        Err(why) => panic!("couldn't create {}: {}", path.display(), why),
        Ok(file) => file,
...
pub fn generate_subject_csv(filename: &str, field_headings: Vec<&str>, data: Vec<Vec<(i32, &str, i32, String, i32, &str, String)>>) {
    let path: &Path = Path::new(filename);
    let mut file: File = match File::create(&path) {
        Err(why) => panic!("couldn't create {}: {}", path.display(), why),
        Ok(file) => file,
